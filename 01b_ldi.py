...
if __name__ == "__main__":
    ...
    if True: # @ TODO: Karim disabling skipping for now. Can you fix it later ?
        ...
        for i in range(config.num_dreams):
            ...
            inpaint_mask_bool = my_utils.pil_mask_to_numpy_bool(inpaint_mask_pil)
            # Optional dilation of the inpainting mask

            print("inpaint_mask_bool sum before dilation:", np.sum(inpaint_mask_bool))
            if config.phase_ldi.depth_inpainting.additionnal_mask_dilation_px > 0:
                inpaint_mask_bool = my_utils.dilate_mask(
                    inpaint_mask_bool,
                    pixels=config.phase_ldi.depth_inpainting.additionnal_mask_dilation_px
                )
                inpaint_mask_pil = my_utils.numpy_bool_to_pil_mask(inpaint_mask_bool)
                print("inpaint_mask_bool sum after dilation:", np.sum(inpaint_mask_bool))

            depth_masked = depth_origin.copy()
            bg_mask_bool = my_utils.pil_mask_to_numpy_bool(inpaint_mask_pil)
            ...
            if config.phase_ldi.depth_inpainting.method == "infusion":
                ...
                depth_inpainted = rgbd_inpaint.interpolate_depth_nearest(
                    depth=depth_masked,
                    bg_mask=bg_mask_bool,
                )
            elif config.phase_ldi.depth_inpainting.method == "bilinear_plus_nn":
                depth_inpainted = rgbd_inpaint.interpolate_depth_bilinear_plus_nn(
                    depth=depth_masked,
                    bg_mask=bg_mask_bool,
                )
            else:
            ...
            depth_inpainted = rgbd_inpaint.ensure_bg_depth_behind_fg(
